		...
		if(isset($_SESSION['LOGGEDIN']) && isset($_SESSION['SID'])) {
			if($_SESSION['GID'] < 3000) {
				...
				if(isset($_POST['positionId']) && isset($_POST['positionName'])) {
					...
					$time = $row['dateTime'];
					$query = "INSERT INTO position(dateTime, positionId, positionName, status)
								VALUES('$time', '$positionId', '$positionName', 'Active')";
					$result = mysql_query($query);
					if(!$result) die ("Table access failed: " . mysql_error());
					if($result) {
						// Position created and redirected to previous page.
						$_SESSION['STATUS'] = 18;
